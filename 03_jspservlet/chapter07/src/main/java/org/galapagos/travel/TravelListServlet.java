...
public class TravelListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("name", names); // 2 이 정보가 들어감 - request scope
		// JSP 이동
		RequestDispatcher dis = request.getRequestDispatcher("travel_list2.jsp"); // 상대경로
		dis.forward(request, response); // 1 얘를 통해서
